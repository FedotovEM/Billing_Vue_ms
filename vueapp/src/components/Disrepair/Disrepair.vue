<template>
    <div @click.left="clearSelect()">
        ...
        <div class="p-3">
            <RouterLink class="btn btn-outline-primary" to="/disrepair/add">Добавить новую запись</RouterLink>
        </div>
        <table id="table_id" class="table table-hover table-bordered" style="width:100%">
            ...
            <tbody>
                <tr v-for="item in disrepairList" :key="item.failureCd" @contextmenu.prevent="showActions($event, item)">
                    <th scope="row">{{item.failureCd}}</th>
                    <td> {{item.failureName}} </td>                    
                </tr>
            </tbody>
        </table>
        <ActionsMenu v-if="showMenu" :itemId="selectedItem.failureCd" :items="disrepairsLink" @close="showMenu = false" :style="{ top: `${y}px`, left: `${x}px` }" />
    </div>
</template>
<script>
    ...
    export default {
        data() {
            return {
                ...
                y: 0,
                disrepairsLink: [
                    { title: 'Редактировать', nameComponent: 'editdisrepair' },
                    { title: 'Удалить', nameComponent: 'deldisrepair' },
                ]
            };
        },
        components: {
