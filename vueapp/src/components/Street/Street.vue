<template>
    <div @click.left="clearSelect()">
        ...
        <select id="IdCombobox" class="form-select-sm">
            <option>Наименование улицы</option>
        </select>

        <input type="text" class="form-control-sm" id="Input" placeholder="Поиск" @keyup="filter" />

        <div class="p-3">
            <RouterLink class="btn btn-outline-primary" to="/street/add">Добавить новую запись</RouterLink>
        </div>
        <table id="table_id" class="table table-hover table-bordered" style="width:100%">
            <thead class="thead-light">
                <tr>
                    <th scope="col" data-type="int" @click="sort">#</th>
                    <th scope="col" data-type="string">Наименование улицы</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item in streetList" :key="item?.streetCd" @contextmenu.prevent="showActions($event, item)">
                    <th scope="row">{{item?.streetCd}}</th>
                    <td> {{item?.streetName}} </td>
                </tr>
            </tbody>
        </table>
        <ActionsMenu v-if="showMenu" :itemId="selectedItem.streetCd" :items="streetsLink" @close="showMenu = false" :style="{ top: `${y}px`, left: `${x}px` }" />
    </div>
</template>
<script>
    ...
    export default {
        data() {
            return {
                ...
                y: 0,
                streetsLink: [
                    { title: 'Редактировать', nameComponent: 'editstreet' },
                    { title: 'Удалить', nameComponent: 'delstreet' },
                ]
            };
        },
        components: {
