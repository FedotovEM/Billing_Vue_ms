<template>
    <div @click.left="clearSelect()">
        ...
        <div class="p-3">
            <RouterLink class="btn btn-outline-primary" to="/executor/add">Добавить новую запись</RouterLink>
        </div>
        <table id="table_id" class="table table-hover table-bordered" style="width:100%">
            <thead class="thead-light">
                <tr>
                    <th scope="col" data-type="int" @click="sort">#</th>
                    <th scope="col" data-type="string">Исполнитель</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item in executorList" :key="item.executorCd" @contextmenu.prevent="showActions($event, item)">
                    <td scope="row">{{item.executorCd}}</td>
                    <td> {{item.fio}} </td>
                </tr>
            </tbody>
        </table>
        <ActionsMenu v-if="showMenu" :itemId="selectedItem.executorCd" :items="executorsLink" @close="showMenu = false" :style="{ top: `${y}px`, left: `${x}px` }"/>
    </div>
</template>
<script>
    ...
    export default {
         data() {
            return {
                ...
                y: 0,
                executorsLink: [
                    { title: 'Редактировать', nameComponent: 'editexecutor' },
                    { title: 'Удалить', nameComponent: 'delexecutor' },
                ]
            };
         },
        components: {
