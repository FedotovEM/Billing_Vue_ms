<template>
    <div @click.left="clearSelect()">
        ...
        <select id="IdCombobox" class="form-select-sm">
            ...
            <option>Значение цены</option>
        </select>

        <input type="text" class="form-control-sm" id="Input" placeholder="Поиск" @keyup="filter" />

        <div class="p-3">
            <RouterLink class="btn btn-outline-primary" to="/price/add">Добавить новую запись</RouterLink>
        </div>
        <table id="table_id" class="table table-hover table-bordered" style="width:100%">
            <thead class="thead-light">
                <tr>
                    ...
                    <th class="col-sm-1" scope="col" data-type="double">Значение цены</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item in priceList" :key="item.priceCd" @contextmenu.prevent="showActions($event, item)">
                    <th scope="row">{{item.priceCd}}</th>
                    <td> {{item.modeName}} </td>
                    <td> {{item.priceValue}} </td>
                </tr>
            </tbody>
        </table>
        <ActionsMenu v-if="showMenu" :itemId="selectedItem.priceCd" :items="pricesLink" @close="showMenu = false" :style="{ top: `${y}px`, left: `${x}px` }" />
    </div>
</template>
<script>
    ...
    export default {
        data() {
            return {
                ...
                y: 0,
                pricesLink: [
                    { title: 'Редактировать', nameComponent: 'editprice' },
                    { title: 'Удалить', nameComponent: 'delprice' },
                ]
            };
        },
        components: {
...
<script setup>
    ...
    const priceList = ref([])
    onMounted(() => {
        axios.get(urls.nachServ + "/Prices", { headers: authHeader() })
            .then((response) => {
